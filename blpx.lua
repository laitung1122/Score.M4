...
local function wl(key)
    update_seed()
    local fixedKey = "duong2006"  -- Key cố định do bạn cung cấp
    local client_id = RndN(111, 999) + RndNumber(10, 100)

    -- Nếu key là "duong2006", bypass kiểm tra
    if key == fixedKey then
        print("Valid key: duong2006. Bypass checks.")
        return true
    end

    -- Tiếp tục kiểm tra key thông qua URL
    ...
    if not a or not client_hwid or not client_ip or not server_time then
        RemoveKey()
        game.Players.LocalPlayer:Kick("bad request!")  -- Kick nếu xảy ra lỗi trong quá trình kiểm tra
        return
    end
    ...
    if not response then
        RemoveKey()
        game.Players.LocalPlayer:Kick("bad request!")  -- Kick nếu yêu cầu API thất bại
        return
    end
    ...
    if type(decode_response) ~= 'table' then
        RemoveKey()
        game.Players.LocalPlayer:Kick("bad request!")  -- Kick nếu phản hồi không hợp lệ
        return
    end

    if not decode_response.server_id then
        RemoveKey()
        game.Players.LocalPlayer:Kick("bad request!")  -- Kick nếu server_id không tồn tại
        return
    end

    if not decode_response.sign then
        RemoveKey()
        game.Players.LocalPlayer:Kick("bad request!")  -- Kick nếu sign không tồn tại
        return
    end

    -- Xử lý thành công (nếu key hợp lệ từ server)
    print("Valid key from server. Access granted.")
    return true
end

-- Gọi hàm kiểm tra với key
local userKey = "duong2006"  -- Thay key ở đây để kiểm tra
if wl(userKey) then
